...
career_probabilities = career_model.predict_proba(
    input_data
)[0]

career_df = pd.DataFrame({
    "Career": label_encoder.classes_,
    "Probability": career_probabilities
})

career_df["Probability"] = (
    career_df["Probability"] * 100
)
...
for i, row in top_3.iterrows():
    ...
    st.write(
        f"### {row['Career']} — {row['Probability']:.1f}% Match"
    )
